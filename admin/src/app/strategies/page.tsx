...
const STRATEGY_CATEGORIES: StrategyCategory[] = [
  ...
  {
    id: 'frameworks',
    name: '⚙️ Framework Modules',
    description: 'Risk management and position sizing tools that enhance ALL strategies',
    icon: Shield,
    color: 'from-slate-500 to-zinc-500',
    strategies: [
      {
        id: 'kelly_position_sizer',
        name: 'Kelly Position Sizer',
        description: 'Optimal bet sizing based on edge and bankroll',
        howItWorks: 'Calculates mathematically optimal position size using Kelly Criterion formula. Prevents over-betting and maximizes long-term growth.',
        confidence: 95,
        expectedReturn: 'Optimizes all strategies',
        riskLevel: 'low',
        configKey: 'kelly_position_sizing_enabled',
        icon: Target,
        color: 'bg-blue-500/20 text-blue-400',
        platforms: ['All'],
        keyPoints: [
          'Framework: Enhances all trading strategies',
          'Prevents over-betting on any single trade',
          'Maximizes geometric growth of bankroll',
          'Uses win rate + average win/loss ratio',
          'Configurable Kelly fraction (default 25%)',
          'Academic backing: proven optimal long-term',
        ],
        workflow: [
          'Estimate edge probability for each trade',
          'Calculate full Kelly: edge / odds',
          'Apply Kelly fraction (e.g., 25% of full Kelly)',
          'Cap at max position size',
          'Execute trade with calculated size',
          'Track actual vs expected for recalibration',
        ],
        requirements: ['Historical win/loss data for calibration'],
        settings: [
          { key: 'kelly_fraction', label: 'Kelly Fraction', type: 'number', defaultValue: 0.25, min: 0.1, max: 1.0, step: 0.05, tooltip: 'Fraction of full Kelly (0.25 = quarter Kelly, safer)' },
          { key: 'kelly_max_position_pct', label: 'Max Position %', type: 'number', defaultValue: 10, min: 1, max: 25, step: 1, tooltip: 'Max % of bankroll per trade' },
        ],
      },
      {
        id: 'regime_detector',
        name: 'Regime Detector',
        description: 'Identifies market conditions to adapt strategy behavior',
        howItWorks: 'Analyzes volatility, trend, and correlation to classify market regime. Adjusts strategy parameters based on current conditions.',
        confidence: 80,
        expectedReturn: 'Reduces drawdowns 20-40%',
        riskLevel: 'low',
        ...
        platforms: ['All'],
        keyPoints: [
          'Framework: Adapts all strategies to market conditions',
          'Identifies 4 regimes: Bull, Bear, Range, Crisis',
          'Uses volatility, trend, correlation signals',
          'Reduces position sizes in high-risk regimes',
          'Increases aggression in favorable regimes',
          'Prevents trading against market structure',
        ],
        workflow: [
          'Calculate rolling volatility (20-day)',
          'Determine trend direction (50/200 MA)',
          'Measure cross-asset correlation',
          'Classify regime based on signals',
          'Broadcast regime to all active strategies',
          'Strategies adjust parameters accordingly',
        ],
        requirements: ['Market data feed'],
        settings: [
          { key: 'regime_lookback_days', label: 'Lookback (days)', type: 'number', defaultValue: 20, min: 5, max: 60, step: 5 },
          { key: 'regime_volatility_threshold', label: 'High Vol Threshold', type: 'number', defaultValue: 2.0, min: 1.0, max: 3.0, step: 0.1, tooltip: 'Std deviations above mean = high volatility' },
        ],
      },
      {
        id: 'circuit_breaker',
        name: 'Circuit Breaker',
        description: 'Emergency stop on excessive drawdown or losses',
        howItWorks: 'Monitors portfolio drawdown in real-time. Halts all trading if daily or total drawdown exceeds threshold. Requires manual reset.',
        confidence: 99,
        expectedReturn: 'Prevents catastrophic losses',
        riskLevel: 'low',
        ...
        platforms: ['All'],
        keyPoints: [
          'Framework: Protects entire portfolio',
          'Daily drawdown limit (default 5%)',
          'Total drawdown limit (default 15%)',
          'Consecutive loss streak protection',
          'Requires manual reset after trigger',
          'Sends immediate notification on trigger',
        ],
        workflow: [
          'Track starting balance at day start',
          'Monitor real-time P&L across all positions',
          'Calculate current drawdown %',
          'If drawdown > threshold: HALT ALL TRADING',
          'Cancel all open orders',
          'Send notification to user',
          'Require manual reset to resume',
        ],
        requirements: ['Real-time balance tracking'],
        settings: [
          { key: 'circuit_breaker_daily_limit_pct', label: 'Daily Loss Limit %', type: 'number', defaultValue: 5, min: 1, max: 20, step: 1 },
          { key: 'circuit_breaker_total_limit_pct', label: 'Total Drawdown Limit %', type: 'number', defaultValue: 15, min: 5, max: 50, step: 5 },
          { key: 'circuit_breaker_consecutive_losses', label: 'Consecutive Loss Limit', type: 'number', defaultValue: 5, min: 3, max: 10, step: 1 },
        ],
      },
      {
        id: 'correlation_tracker',
        name: 'Correlation Tracker',
        description: 'Prevents over-concentration in correlated positions',
        howItWorks: 'Tracks correlation between all open positions. Limits exposure to highly correlated assets to prevent concentrated losses.',
        confidence: 85,
        expectedReturn: 'Reduces tail risk',
        riskLevel: 'low',
        configKey: 'correlation_tracking_enabled',
        icon: GitBranch,
        color: 'bg-teal-500/20 text-teal-400',
        platforms: ['All'],
        keyPoints: [
          'Framework: Portfolio-level risk management',
          'Calculates pairwise correlations',
          'Limits total exposure to correlated assets',
          'Prevents "hidden concentration" risk',
          'Real-time monitoring and alerts',
          'Especially important for crypto portfolios',
        ],
        workflow: [
          'Calculate rolling correlation matrix',
          'For each new position, check correlation with existing',
          'If correlation > threshold with existing positions:',
          'Reduce new position size proportionally',
          'Or block trade if portfolio too concentrated',
          'Alert user on high correlation warning',
        ],
        requirements: ['Historical price data for correlation'],
        settings: [
          { key: 'correlation_threshold', label: 'Correlation Threshold', type: 'number', defaultValue: 0.7, min: 0.3, max: 0.95, step: 0.05, tooltip: 'Block/reduce if correlation exceeds this' },
          { key: 'correlation_max_cluster_pct', label: 'Max Cluster Exposure %', type: 'number', defaultValue: 30, min: 10, max: 50, step: 5, tooltip: 'Max % of portfolio in correlated assets' },
        ],
      },
    ],
  },
